...
colnames(pa.recovered.L.phi.naive)
table(pa.recovered.L.phi.naive$Vertex.to.Recover)
...
df[3,] <- c(50000, 5, tmp[1,seq(3,51,by=2)])
df[4,] <- c(50000, 10, tmp[2,seq(5,101,by=4)])
...
df.melted %>%
  ...
  geom_line(aes(linetype = L), size = 2) +
  theme_fivethirtyeight() +
  scale_x_continuous(limits = c(0, 500), labels = K.s, breaks = K.s) +
  scale_y_continuous(limits = c(0,.8), labels = proportion.cuts, breaks = proportion.cuts) +
  labs(subtitle = 'Graph size: 50,000; Over different K vertices returned in total;',
       title = 'Pref. Attm. Model on Tree:
                Recovery of The First 5 and 10 Vertices
                Comparison: Local Search vs. Simple Search',
       x = 'K (total)', y = 'Proportion',
       color = 'Method',
